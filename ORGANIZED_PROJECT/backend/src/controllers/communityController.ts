...
export const getCommunity = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { page = 1, limit = 20 } = req.query;
  const offset = (Number(page) - 1) * Number(limit);
  const { data: items, error, count } = await supabase.from('posts').select('*', { count: 'exact' }).order('created_at', { ascending: false }).range(offset, offset + Number(limit) - 1);
...
export const getCommunityById = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const { data: item, error } = await supabase.from('posts').select('*').eq('id', id).single();
...
export const createCommunity = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const { data: item, error } = await supabase.from('posts').insert({...req.body, user_id: userId}).select().single();
...
export const updateCommunity = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const { data: item, error } = await supabase.from('posts').update(req.body).eq('id', id).select().single();
